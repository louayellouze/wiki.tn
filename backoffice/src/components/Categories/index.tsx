                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                            <div className={`text-sm font-bold tracking-tight uppercase transition-colors ${level === 0 ? "text-gray-900 dark:text-white" : "text-gray-700 dark:text-gray-300"}`}>
                                {category.name} <span className="ml-1 text-[10px] font-mono text-gray-400 font-normal">#{category.id}</span>
                            </div>
                            {category.isFeatured && (
...
const Categories = () => {
        ...
        try {
            if (viewMode === "tree") {
                ...
                if (response && 'content' in response) {
                    setFlatCategories(response.content);
                    setTotalPages(response.totalPages);
                    setTotalElements(response.totalElements);
                } else {
                    setFlatCategories(response as Category[]);
                }
            }
        } catch (error) {
    ...
    const handleOpenCreate = () => {
        setModalMode("create");
        setFormData({ name: "", description: "", imageUrl: "", isFeatured: false, parentId: undefined });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Invalidate front-office category tree cache so new categories appear immediately
            localStorage.removeItem('category_tree_cache');
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // Invalidate front-office category tree cache
            localStorage.removeItem('category_tree_cache');
    ...
    return (
        <div className="mx-auto max-w-full py-4 sm:py-6">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center rounded-lg border border-gray-200 bg-white p-1 dark:border-gray-700 dark:bg-gray-800 shadow-sm">
                        <button
                            ...
                            className={`flex h-9 w-9 items-center justify-center rounded-md transition-all ${viewMode === "tree" ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700"}`}
                        ...
                        <button
                            ...
                            className={`flex h-9 w-9 items-center justify-center rounded-md transition-all ${viewMode === "list" ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700"}`}
                            title="Vue liste"
                        >
                            <List size={18} />
                        </button>
                                    ...
                                    {flatCategories
                                        ...
                                        <tr key={cat.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    {cat.imageUrl ? (
                                                        <img src={cat.imageUrl} className="h-10 w-10 rounded border object-cover" />
                                                    ) : (
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="font-mono text-[10px] text-indigo-600">#{cat.id}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button
                                                        onClick={() => {
                                                            window.location.href = `/hero-banners?categoryId=${cat.id}`;
                                                        }}
            ...
            {viewMode === "list" && !loading && totalPages > 1 && (
                <div className="mt-6 flex flex-col items-center justify-between gap-4 border-t border-gray-200 pt-6 dark:border-gray-700 sm:flex-row">
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        ...
                        sur <span className="font-semibold text-gray-900 dark:text-white">{totalElements}</span> catégories
                    </p>
            ...
            {isEditModalOpen && (
                ...
                >
                    <div 
                        className="w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl dark:bg-gray-800 max-h-[90vh] overflow-y-auto"
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="flex items-center gap-2 rounded-lg border border-yellow-100 bg-yellow-50/30 p-3 dark:border-yellow-900/20 dark:bg-yellow-900/10">
                                <input
                                    type="checkbox"
                                    id="isFeatured"
                                ...
                                />
                                <label htmlFor="isFeatured" className="flex items-center gap-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                                    <Star size={14} className={formData.isFeatured ? "fill-yellow-400 text-yellow-400" : "text-gray-400"} />
