            ...
            <div className="flex flex-wrap gap-4 justify-center">
              {product.images?.map((img, index) => (
                <button
                  ...
                  className={`w-24 h-24 rounded-2xl bg-slate-50 border-2 transition-all p-2 overflow-hidden flex items-center justify-center ${mainImage === img.imageUrl ? 'border-wiki-btn shadow-md scale-105' : 'border-slate-100 hover:border-slate-300'}`}
                >
                  <img className="max-h-full object-contain" src={img.imageUrl || '/assets/img/2-1.png'} alt={`${product.title} - ${index}`} />
                </button>
              ))}
            </div>
          </div>
