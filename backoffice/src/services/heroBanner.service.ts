...
export const getHeroBanners = async (page?: number, size?: number): Promise<PageResponse<HeroBanner> | HeroBanner[]> => {
    if (page !== undefined && size !== undefined) {
        return apiFetch<PageResponse<HeroBanner>>(`/v1/hero-banners/paginated?page=${page}&size=${size}`);
    }
