                    ...
                    <div className='flex flex-wrap items-center'>
                        ...
                        >
                            <div
                                ...
                                className="ml-16 w-52 h-16 bg-wiki flex items-center justify-center hover:bg-wiki-dark cursor-pointer transition-colors"
                            >
                                <div className="text-white font-semibold text-base mr-3">Browse categories</div>
                                <svg
                                    width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg"
                                    className={`transition-transform duration-300 ${isMenuOpen || isManualOpen ? 'rotate-180' : ''}`}
                                >
                            ...
                            <div className={`absolute left-16 w-[1000px] h-[550px] bg-white/95 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] z-50 top-16 border border-slate-200 rounded-b-2xl overflow-hidden transition-all duration-300 flex ${isMenuOpen || isManualOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
                                ...
                                <div className="w-1/3 border-r border-slate-100 h-full overflow-y-auto bg-slate-50/50 p-4 scrollbar-thin scrollbar-thumb-slate-200">
                                    <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 px-2">Rayons</div>
                                    {categories.map(category => (
                                        <Link
                                            ...
                                            className={`p-3 px-4 cursor-pointer flex justify-between items-center text-sm font-bold transition-all duration-200 rounded-xl mb-1 ${activeCategory?.id === category.id ? 'bg-wiki text-white shadow-md shadow-wiki/20' : 'text-slate-600 hover:bg-white hover:shadow-sm hover:text-wiki'}`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${activeCategory?.id === category.id ? 'bg-white/20' : 'bg-slate-100 text-slate-400'}`}>
                                            ...
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={activeCategory?.id === category.id ? 'opacity-100' : 'opacity-0 -translate-x-2 transition-all'}>
                                ...
                                <div className="w-2/3 p-8 h-full overflow-y-auto bg-white scrollbar-thin scrollbar-thumb-slate-200">
                                    {activeCategory ? (
                                        <div className="animate-in fade-in slide-in-from-left-4 duration-300">
                                            <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-100">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-slate-500 text-sm mt-1">{activeCategory.subCategories?.length || 0} sous-rayons disponibles</p>
                                                </div>
                                            </div>
                                                ...
                                                {activeCategory.subCategories && activeCategory.subCategories.length > 0 ? (
                                                    activeCategory.subCategories.map(sub => (
                                                        <div key={sub.id} className="relative">
                                                            {/* Line connector for visual hierarchy */}
                                                            <div className="absolute left-4 top-10 bottom-2 w-px bg-slate-100 -z-10" />
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-6 animate-in fade-in duration-500">
                                            ...
                                            <p className="font-bold text-slate-400 uppercase tracking-widest text-sm">Survolez un rayon</p>
                                        </div>
                                    )}
                                </div>
                            </div>
