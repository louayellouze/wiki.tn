            ...
            {NAV_DATA.map((section) => {
              ...
              const filteredItems = section.items.map(item => {
                ...
                if (!item.items || item.items.length === 0) {
                  if (item.title === "Utilisateurs") return (role.includes("ADMIN") || role.includes("INFOLINE")) ? item : null;
                  if (item.title === "Historique") return role.includes("ADMIN") ? item : null;
                  return item;
                }

                // Handle items with children (like "Tables")
                const filteredChildren = item.items.filter(subItem => {
                  if (subItem.title === "Utilisateurs") return role.includes("ADMIN") || role.includes("INFOLINE");
                ...
                if (item.items.length === 0 && ["Dashboard", "Forms", "Pages"].includes(item.title)) return false;
                return true;
