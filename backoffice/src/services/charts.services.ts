...
export async function getPaymentsOverviewData(
  ...
  try {
    ...
    orders.forEach(order => {
      ...
      // In a real scenario, we might have payment status. 
      // For now, let's treat DELIVERED as received and PENDING as due.
      if (order.status === "DELIVERED") {
        receivedMap[month] += order.totalAmount;
      } else if (order.status === "PENDING" || order.status === "SHIPPED") {
        dueMap[month] += order.totalAmount;
      }
    });

    return {
      received: months.map(m => ({ x: m, y: receivedMap[m] })),
      due: months.map(m => ({ x: m, y: dueMap[m] })),
    };
  } catch (error) {
