...
const SideSpecFilter: React.FC<SideSpecFilterProps> = ({
    ...
    useEffect(() => {
        const fetchSpecKeys = async () => {
            setLoading(true);
            try {
                let url = '/v1/spec-keys';
                if (searchQuery) {
                    url = `/v1/spec-keys/search?q=${encodeURIComponent(searchQuery)}`;
                } else if (categoryId) {
                    url = `/v1/spec-keys/category/${categoryId}`;
                }

                const res = await api.get<SpecKey[]>(url);
                setSpecKeys(res.data);

                const valuesMap: Record<string, string[]> = {};
                await Promise.all(res.data.map(async (key) => {
                    try {
                        let vUrl = `/v1/spec-keys/${key.id}/values`;
                        const params = new URLSearchParams();
                        if (categoryId) params.append('categoryId', categoryId);
                        if (searchQuery) params.append('q', searchQuery);

                        const finalVUrl = params.toString() ? `${vUrl}?${params.toString()}` : vUrl;
                        const vRes = await api.get<string[]>(finalVUrl);
                        if (vRes.data.length > 0) valuesMap[key.name] = vRes.data;
                    } catch { /* skip */ }
                }));
