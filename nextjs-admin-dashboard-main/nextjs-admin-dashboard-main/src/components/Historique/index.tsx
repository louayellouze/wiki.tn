...
const HistoriqueComponent = () => {
    ...
    useEffect(() => {
        fetchLogs();
    }, [filters.entityType, filters.actionType]); // Reload on select change, debounced for text

    const fetchLogs = async () => {
        setLoading(true);
        try {
            // Use searchHistorique which handles filtering and pagination content
            const data = await searchHistorique(filters);
            setLogs(data);
        } catch (error) {
        ...
        <div className="mx-auto max-w-7xl p-4 sm:p-6">
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div className="hidden border-b border-gray-200 bg-gray-50 px-6 py-4 dark:border-gray-700 dark:bg-gray-800/50 md:block">
                    <div className="grid grid-cols-[180px_120px_150px_1fr] gap-4">
                    ...
                    {loading ? (
                        ...
                        filteredLogs.map((log) => (
                            <div key={log.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors md:px-6">
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-[180px_120px_150px_1fr]">
